...
DB.exec("ALTER TABLE bayboxes DROP COLUMN IF EXISTS last_ok_at")
DB.setup!
...
# --- a baybox, with its private key encrypted at rest ----------------------
...
check("one baybox per user", dup, "refused")
...
check("job points at the baybox", job["baybox_id"], box["id"])

second = begin
  DB.exec(<<~SQL, ["furkansahin", box["id"], "ubicloud/ubicloud", 6172, "rq-ubicloud-6172"])
    INSERT INTO review_jobs (login, baybox_id, repo, pr_number, box_name) VALUES ($1,$2,$3,$4,$5)
  SQL
  "inserted"
rescue PG::UniqueViolation
  "refused"
...
other = DB.row(<<~SQL, ["mohi-kalantari", "10.0.0.6", Crypto.encrypt("k"), "pub"])
  INSERT INTO bayboxes (login, host, private_key_enc, public_key) VALUES ($1,$2,$3,$4) RETURNING *
SQL
o = begin
...
DB.exec("DELETE FROM bayboxes WHERE id = $1", [other["id"]])
check("jobs survive removing a baybox",
      DB.row("SELECT count(*)::int n FROM review_jobs WHERE login=$1", ["mohi-kalantari"])["n"], 1)
check("the link is cleared, not orphaned",
      DB.row("SELECT baybox_id FROM review_jobs WHERE login=$1", ["mohi-kalantari"])["baybox_id"], nil)
